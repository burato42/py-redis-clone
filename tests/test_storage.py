...
class TestStorage:
    ...
    async def test_stream_xadd(self, storage):
        with pytest.raises(
            ...
            await storage.set_stream(
                "key1", Value({"id": "0-0", "foo": "bar", "baz": "qux"})
            )
        ...
        with pytest.raises(
            ...
            await storage.set_stream(
                "key1", Value({"id": "0-1", "bar": "foo", "baz": "qux"})
            )
    ...
    async def test_stream_xadd_autogenerated_id(self, storage):
        ...
        with pytest.raises(
            ...
            await storage.set_stream(
                "key1", Value({"id": "1-*", "foo": "bar", "baz": "qux"})
            )
    ...
    async def test_stream_range_inclusive(self, storage):
        ...
        await storage.set_stream(
            "key1", Value({"id": "0-2", "foo": "bar", "baz": "qux"})
        )
        await storage.set_stream(
            "key1", Value({"id": "1-1", "foo": "bar", "baz": "qux"})
        )
        await storage.set_stream(
            "key1", Value({"id": "1-3", "foo": "bar", "baz": "qux"})
        )
        await storage.set_stream(
            "key1", Value({"id": "2-1", "foo": "bar", "baz": "qux"})
        )
        assert await storage.get_stream_range("key1", (0, 2), (1, 3), True) == [
    ...
    async def test_stream_range_blocking_timeout(self, storage):
        ...
        value, _ = await asyncio.gather(
            storage.get_stream_range("key1", (0, 2), (1, 3), True, 500),
            set_after_delay(),
        )
    ...
    @pytest.mark.asyncio
    async def test_stream_range_zero_blocking(self, storage):
        async def set_after_delay():
            await asyncio.sleep(0.1)
            await storage.set_stream(
                "key1", Value({"id": "0-1", "foo": "bar", "baz": "qux"})
        ...
        value, _ = await asyncio.gather(
            storage.get_stream_range("key1", (0, 2), (1, 3), True, 0),
            set_after_delay(),
        )
