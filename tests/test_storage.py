...
class TestStorage:
    ...
    @pytest.mark.asyncio
    async def test_stream_xadd(self, storage):
        with pytest.raises(
            ValueError, match="The ID specified in XADD must be greater than 0-0"
        ):
            await storage.set_stream("key1", Value({"id": "0-0", "foo": "bar", "baz": "qux"}))

        await storage.set_stream("key1", Value({"id": "0-1", "foo": "bar", "baz": "qux"}))
        ...
        with pytest.raises(
            ...
            await storage.set_stream("key1", Value({"id": "0-1", "bar": "foo", "baz": "qux"}))

        await storage.set_stream("key1", Value({"id": "1-1", "bar": "foo", "baz": "qux"}))
        assert storage.data.get("key1") == deque(
            [
                Value({"id": "0-1", "foo": "bar", "baz": "qux"}),
    ...
    @pytest.mark.asyncio
    async def test_stream_xadd_autogenerated_id(self, storage):
        await storage.set_stream("key1", Value({"id": "2-*", "foo": "bar", "baz": "qux"}))
        assert storage.data.get("key1") == deque(
        ...
        with pytest.raises(
            ...
            await storage.set_stream("key1", Value({"id": "1-*", "foo": "bar", "baz": "qux"}))

    @pytest.mark.asyncio
    async def test_stream_xadd_full_autogenerated_id(self, storage, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_stream_range_inclusive(self, storage):
        assert await storage.get_stream_range("key1", (0, 2), (1, 3), True) == []
        await storage.set_stream("key1", Value({"id": "0-1", "foo": "bar", "baz": "qux"}))
        await storage.set_stream("key1", Value({"id": "0-2", "foo": "bar", "baz": "qux"}))
        await storage.set_stream("key1", Value({"id": "1-1", "foo": "bar", "baz": "qux"}))
        await storage.set_stream("key1", Value({"id": "1-3", "foo": "bar", "baz": "qux"}))
        await storage.set_stream("key1", Value({"id": "2-1", "foo": "bar", "baz": "qux"}))
        assert await storage.get_stream_range("key1", (0, 2), (1, 3), True) == [
    ...
    @pytest.mark.asyncio
    async def test_stream_range_exclusive(self, storage):
        assert await storage.get_stream_range("key1", (0, 2), (1, 3), False) == []
        await storage.set_stream("key1", Value({"id": "0-1", "foo": "bar", "baz": "qux"}))
        await storage.set_stream("key1", Value({"id": "0-2", "foo": "bar", "baz": "qux"}))
        await storage.set_stream("key1", Value({"id": "1-1", "foo": "bar", "baz": "qux"}))
        await storage.set_stream("key1", Value({"id": "1-3", "foo": "bar", "baz": "qux"}))
    ...
    @pytest.mark.asyncio
    async def test_stream_range_blocking_timeout(self, storage):
        async def set_after_delay():
            await asyncio.sleep(1.01)
            await storage.set_stream("key1", Value({"id": "0-1", "foo": "bar", "baz": "qux"}))
            await storage.set_stream("key1", Value({"id": "0-2", "foo": "bar", "baz": "qux"}))
        ...
        value, _ = await asyncio.gather(
            storage.get_stream_range("key1", (0, 2), (1, 3), True, 500), set_after_delay()
        )

        assert value == []

    @pytest.mark.asyncio
    async def test_stream_range_blocking(self, storage):
        async def set_after_delay():
